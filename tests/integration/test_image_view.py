    ...
    return bytes(
        [
            ...
            *([16] * 64),  # quantisation table (uniform)
            ...
            0x01,
            0x11,
            0x00,  # Cb: sampling + qt id
            0xFF,
            0xC4,  # DHT marker
            0x00,
            0x1F,
            0x00,  # DHT length + class/id
            0x00,
            0x01,
            0x05,
            0x01,
            ...
            0x01,  # 1 component
            0x01,
            0x00,  # component id + AC/DC table ids
            0x00,
            0x3F,
            0x00,  # Ss, Se, Ah/Al
...
def _make_minimal_webp() -> bytes:
    ...
    vp8l_data = bytes(
        [
            ...
            0x00,
            0x00,
            0x00,  # 1×1 (width-1=0 in bits 0-13, height-1=0 in bits 14-27)
            0x00,
            0xFF,
            0xFF,
            0xFF,
            0xFF,  # ARGB = 0xFFFFFFFF (white, fully opaque)
        ]
    )
...
class TestAutodetect:
    ...
    def test_multiple_frames_same_format(
        self, api_client, device_simulator, clean_state, fmt, builder
    ):
        ...
        assert status[
            "isConnected"
        ], f"Device must stay connected after 10 {fmt} frames"
    ...
    def test_mixed_formats_autodetect(self, api_client, device_simulator, clean_state):
        ...
        assert status[
            "isConnected"
        ], "Device must stay connected after mixed format frames"
...
class TestManualMode:
    ...
    def test_manual_jpeg_with_custom_framing(
        ...
        _setup_image_project(
            api_client, detection_mode="manual", start_hex=start_hex, end_hex=end_hex
        )
    ...
    def test_manual_png_with_custom_framing(
        self, api_client, device_simulator, clean_state
    ):
    ...
    def test_manual_mode_single_byte_delimiters(
        self, api_client, device_simulator, clean_state
    ):
        ...
        assert status[
            "isConnected"
        ], "Device must stay connected with single-byte delimiters"
    ...
    def test_manual_mode_falls_back_to_autodetect_when_sequences_empty(
        ...
        _setup_image_project(
            api_client, detection_mode="manual", start_hex="", end_hex=""
        )
        ...
        assert status[
            "isConnected"
        ], "Should stay connected even with empty manual sequences"
...
class TestCorruptedData:
    ...
    def test_truncated_png_does_not_crash(
        self, api_client, device_simulator, clean_state
    ):
    ...
    def test_corrupted_bmp_oversized_size_field(
        self, api_client, device_simulator, clean_state
    ):
        ...
        assert status[
            "isConnected"
        ], "Oversized BMP claim must not disconnect the device"
    ...
    def test_garbage_then_valid_jpeg(self, api_client, device_simulator, clean_state):
        ...
        assert status[
            "isConnected"
        ], "Device must stay connected after garbage + valid JPEG"
    ...
    def test_jpeg_with_corrupted_body(self, api_client, device_simulator, clean_state):
        ...
        assert status[
            "isConnected"
        ], "Corrupted JPEG body must not disconnect the device"

    def test_accumulator_overflow_recovery(
        self, api_client, device_simulator, clean_state
    ):
        """
        ...
        assert status[
            "isConnected"
        ], "Device must stay connected after accumulator overflow"

    def test_manual_mode_corrupted_payload(
        self, api_client, device_simulator, clean_state
    ):
        ...
        assert status[
            "isConnected"
        ], "Invalid image payload in manual mode must not crash"
