...
class QtVirtualCANClient:
    ...
    def __init__(
        self, host: str = "localhost", port: int = 35468, channel: str = "can0"
    ):
...
class ECUSimulator:
    ...
    def send_frame(self, arbitration_id: int, data: bytes):
        ...
        if self.debug and arbitration_id == 0x100:
            ...
            print(
                f"        Byte 0-1 (RPM raw): 0x{data[0]:02X}{data[1]:02X} = {(data[0]<<8)|data[1]} → {((data[0]<<8)|data[1])*0.25:.1f} RPM"
            )
            print(
                f"        Byte 2-3 (Speed raw): 0x{data[2]:02X}{data[3]:02X} = {(data[2]<<8)|data[3]} → {((data[2]<<8)|data[3])*0.01:.1f} km/h"
            )
    ...
    def encode_engine_rpm_speed(self) -> bytes:
        ...
        rpm_clamped = max(
            DBCRanges.ENGINE_RPM_MIN, min(DBCRanges.ENGINE_RPM_MAX, rpm_with_noise)
        )
        rpm_raw = int(rpm_clamped * 4)

        speed_clamped = max(
            DBCRanges.VEHICLE_SPEED_MIN,
            min(DBCRanges.VEHICLE_SPEED_MAX, self.engine.speed_kmh),
        )
    ...
    def encode_engine_temp_load(self) -> bytes:
        ...
        coolant_clamped = max(
            DBCRanges.COOLANT_TEMP_MIN,
            min(DBCRanges.COOLANT_TEMP_MAX, coolant_with_noise),
        )
        coolant = int(coolant_clamped + 40)

        intake_with_noise = self.engine.intake_temp + self.intake_temp_noise
        intake_clamped = max(
            DBCRanges.INTAKE_TEMP_MIN, min(DBCRanges.INTAKE_TEMP_MAX, intake_with_noise)
        )
        intake = int(intake_clamped + 40)

        load_clamped = max(
            DBCRanges.ENGINE_LOAD_MIN,
            min(DBCRanges.ENGINE_LOAD_MAX, self.engine.engine_load),
        )
    ...
    def encode_engine_throttle(self) -> bytes:
        ...
        """
        throttle_clamped = max(
            DBCRanges.THROTTLE_POS_MIN,
            min(DBCRanges.THROTTLE_POS_MAX, self.engine.throttle_pos),
        )
        throttle = int(throttle_clamped * 2.55)

        fuel_with_noise = self.engine.fuel_pressure + self.fuel_pressure_noise
        fuel_clamped = max(
            DBCRanges.FUEL_PRESSURE_MIN,
            min(DBCRanges.FUEL_PRESSURE_MAX, fuel_with_noise),
        )
    ...
    def encode_transmission(self) -> bytes:
        ...
        temp_clamped = max(
            DBCRanges.TRANSMISSION_TEMP_MIN,
            min(DBCRanges.TRANSMISSION_TEMP_MAX, self.transmission.trans_temp),
        )
    ...
    def encode_bms_voltage_current(self) -> bytes:
        ...
        voltage_clamped = max(
            DBCRanges.PACK_VOLTAGE_MIN,
            min(DBCRanges.PACK_VOLTAGE_MAX, voltage_with_noise),
        )
        voltage_raw = int(voltage_clamped * 10)

        current_with_noise = self.battery.pack_current + self.battery_current_noise
        current_clamped = max(
            DBCRanges.PACK_CURRENT_MIN,
            min(DBCRanges.PACK_CURRENT_MAX, current_with_noise),
        )
    ...
    def encode_wheel_speeds(self) -> bytes:
        ...
        """
        fl_clamped = max(
            DBCRanges.WHEEL_SPEED_MIN,
            min(
                DBCRanges.WHEEL_SPEED_MAX,
                self.chassis.wheel_speed_fl + self.wheel_speed_noise[0],
            ),
        )
        fr_clamped = max(
            DBCRanges.WHEEL_SPEED_MIN,
            min(
                DBCRanges.WHEEL_SPEED_MAX,
                self.chassis.wheel_speed_fr + self.wheel_speed_noise[1],
            ),
        )
        rl_clamped = max(
            DBCRanges.WHEEL_SPEED_MIN,
            min(
                DBCRanges.WHEEL_SPEED_MAX,
                self.chassis.wheel_speed_rl + self.wheel_speed_noise[2],
            ),
        )
        rr_clamped = max(
            DBCRanges.WHEEL_SPEED_MIN,
            min(
                DBCRanges.WHEEL_SPEED_MAX,
                self.chassis.wheel_speed_rr + self.wheel_speed_noise[3],
            ),
        )
    ...
    def encode_brake_pressure(self) -> bytes:
        ...
        pressure_clamped = max(
            DBCRanges.BRAKE_PRESSURE_MIN,
            min(DBCRanges.BRAKE_PRESSURE_MAX, pressure_with_noise),
        )
    ...
    def encode_steering_angle(self) -> bytes:
        ...
        angle_clamped = max(
            DBCRanges.STEERING_ANGLE_MIN,
            min(DBCRanges.STEERING_ANGLE_MAX, angle_with_noise),
        )
    ...
    def encode_body_lights(self) -> bytes:
        ...
        """
        lights = 0
        if self.body.headlights:
            lights |= 0x01
        if self.body.high_beams:
            lights |= 0x02
        if self.body.turn_left:
            lights |= 0x04
        if self.body.turn_right:
            lights |= 0x08
        if self.body.hazards:
            lights |= 0x10
    ...
    def encode_body_doors(self) -> bytes:
        ...
        """
        doors = 0
        if self.body.door_fl:
            doors |= 0x01
        if self.body.door_fr:
            doors |= 0x02
        if self.body.door_rl:
            doors |= 0x04
        if self.body.door_rr:
            doors |= 0x08
        if self.body.trunk:
            doors |= 0x10
    ...
    def encode_fuel(self) -> bytes:
        ...
        """
        level_clamped = max(
            DBCRanges.FUEL_LEVEL_MIN,
            min(DBCRanges.FUEL_LEVEL_MAX, self.cluster.fuel_level),
        )
        level = int(level_clamped)

        consumption_clamped = max(
            DBCRanges.FUEL_CONSUMPTION_MIN,
            min(DBCRanges.FUEL_CONSUMPTION_MAX, self.cluster.fuel_consumption),
        )
    ...
    def _update_noise(self, dt: float):
        ...
        sigma_rpm = 15.0
        self.rpm_noise += theta * (0 - self.rpm_noise) * dt + sigma_rpm * math.sqrt(
            dt
        ) * random.gauss(0, 1)

        sigma_coolant = 0.3
        self.coolant_temp_noise += theta * (
            0 - self.coolant_temp_noise
        ) * dt + sigma_coolant * math.sqrt(dt) * random.gauss(0, 1)

        sigma_intake = 0.5
        self.intake_temp_noise += theta * (
            0 - self.intake_temp_noise
        ) * dt + sigma_intake * math.sqrt(dt) * random.gauss(0, 1)

        sigma_fuel = 3.0
        self.fuel_pressure_noise += theta * (
            0 - self.fuel_pressure_noise
        ) * dt + sigma_fuel * math.sqrt(dt) * random.gauss(0, 1)

        for i in range(4):
            sigma_wheel = 0.2
            self.wheel_speed_noise[i] += theta * (
                0 - self.wheel_speed_noise[i]
            ) * dt + sigma_wheel * math.sqrt(dt) * random.gauss(0, 1)

        sigma_brake = 1.5
        self.brake_pressure_noise += theta * (
            0 - self.brake_pressure_noise
        ) * dt + sigma_brake * math.sqrt(dt) * random.gauss(0, 1)

        sigma_steering = 0.5
        self.steering_noise += theta * (
            0 - self.steering_noise
        ) * dt + sigma_steering * math.sqrt(dt) * random.gauss(0, 1)

        sigma_voltage = 0.3
        self.battery_voltage_noise += theta * (
            0 - self.battery_voltage_noise
        ) * dt + sigma_voltage * math.sqrt(dt) * random.gauss(0, 1)

        sigma_current = 0.5
        self.battery_current_noise += theta * (
            0 - self.battery_current_noise
        ) * dt + sigma_current * math.sqrt(dt) * random.gauss(0, 1)
    ...
    def _update_engine(self, dt: float):
        ...
        self.engine.engine_load = (
            self.engine.throttle_pos * 0.7 + self.engine.rpm / 6500 * 30
        )
    ...
    def _update_transmission(self, dt: float):
        ...
        if speed < 3:
            ...
            target_gear = (
                1 if current_gear <= 1 else (1 if speed < 15 else current_gear)
            )
        elif speed < 38:
            target_gear = (
                2 if current_gear <= 2 else (2 if speed < 32 else current_gear)
            )
        elif speed < 58:
            target_gear = (
                3 if current_gear <= 3 else (3 if speed < 50 else current_gear)
            )
        elif speed < 78:
            target_gear = (
                4 if current_gear <= 4 else (4 if speed < 68 else current_gear)
            )
        elif speed < 98:
            target_gear = (
                5 if current_gear <= 5 else (5 if speed < 88 else current_gear)
            )
        else:
            target_gear = 6
    ...
    def _update_battery(self, dt: float):
        ...
        self.battery.pack_voltage = (
            350 + self.battery.soc * 0.8 - self.battery.pack_current * 0.05
        )
    ...
    def run(self, update_rate: float = 100.0, print_status: bool = True):
        ...
        if not self.can_sender:
            raise RuntimeError(
                "CAN sender not configured. Call set_can_sender() first."
            )
    ...
    def _print_status(self):
        ...
        print(
            f"\rRPM: {self.engine.rpm:5.0f} (raw: {rpm_raw:5d}, 0x{rpm_raw:04X}) | "
...
def main():
    ...
    if args.interface == "virtual":
        ...
        sender, bus = create_python_can_sender(
