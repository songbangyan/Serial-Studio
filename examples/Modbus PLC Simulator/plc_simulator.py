...
class PIDController:
    ...
    def __init__(
        ...
        setpoint: float,
        output_min: float = 0,
        output_max: float = MOTOR_MAX_RPM,
    ):
        self.kp = kp
        self.ki = ki
        self.kd = kd
    ...
    def update(self, measured: float, dt: float) -> float:
        ...
        if dt > 0:
            ...
            self.last_derivative += self.derivative_filter * (
                raw_derivative - self.last_derivative
            )
...
class HydraulicTestStand:
    ...
    def _update_noise(self, dt: float):
        ...
        sigma_pressure = 2.0
        self.pressure_noise += theta * (
            0 - self.pressure_noise
        ) * dt + sigma_pressure * math.sqrt(dt) * random.gauss(0, 1)
        self.pressure_noise = clamp(self.pressure_noise, -8, 8)  # Hard limit noise

        # Temperature noise: ±0.3°F typical
        sigma_temp = 0.2
        self.temp_noise += theta * (0 - self.temp_noise) * dt + sigma_temp * math.sqrt(
            dt
        ) * random.gauss(0, 1)

        # Flow noise: ±0.2 GPM typical
        sigma_flow = 0.15
        self.flow_noise += theta * (0 - self.flow_noise) * dt + sigma_flow * math.sqrt(
            dt
        ) * random.gauss(0, 1)

        # Vibration noise: ±0.3 mm/s typical
        sigma_vib = 0.2
        self.vibration_noise += theta * (
            0 - self.vibration_noise
        ) * dt + sigma_vib * math.sqrt(dt) * random.gauss(0, 1)
    ...
    def _update_hydraulic_physics(self, dt: float):
        ...
        if self.motor_rpm > 100:
            ...
            pulsation += (
                math.sin(self.runtime * 2 * math.pi * piston_freq * 2) * 5
            )  # Harmonic
        else:
            pulsation = 0
    ...
    def _update_flow_and_load(self):
        ...
        pump_flow_lpm = (
            PUMP_DISPLACEMENT * self.motor_rpm * PUMP_VOLUMETRIC_EFF
        ) / 1000.0
        ...
        if self.motor_rpm > 50:
            self.motor_load_pct = (
                pressure_load * speed_factor * 0.85 + mechanical_losses
            ) * 100
        else:
    ...
    def _update_thermal_physics(self, dt: float):
        ...
        pump_flow_m3 = (
            (PUMP_DISPLACEMENT * self.motor_rpm * PUMP_VOLUMETRIC_EFF) / 60.0 * 1e-6
        )
    ...
    def _pressure_test_sequence(self, dt: float):
        ...
        if self.phase_timer < 8.0:
            ...
            # Ramp up RPM
            target_rpm = (
                MOTOR_RATED_RPM + progress * (MOTOR_MAX_RPM - MOTOR_RATED_RPM) * 0.7
            )
    ...
    def _failure_sequence(self, dt: float):
        ...
        if self.failure_timer < FAILURE_DURATION:
            ...
            target_psi = (
                pa_to_psi(self.pre_failure_pressure) + progress * 800
            )  # Rise ~800 PSI
    ...
    def _shutdown_sequence(self, dt: float):
        ...
        if self.pressure_pa > 101325:
            ...
            tau = 2.0
            self.pressure_pa = 101325 + (self.pressure_pa - 101325) * math.exp(
                -dt / tau
            )
    ...
    def get_registers(self) -> list:
        ...
        return [
            ...
            int(
                round(clamp(temp_reading, AMBIENT_TEMP_F, MAX_OIL_TEMP_F + 20))
            ),  # HR[2]: Temp °F
            int(
                round(clamp(pressure_reading, 0, RELIEF_VALVE_PSI))
            ),  # HR[3]: Pressure PSI
            ...
            int(
                round(clamp(vibration_reading * 10, 0, 150))
            ),  # HR[8]: Vibration mm/s x10
            0,  # HR[9]: Spare
        ]
...
async def run_server():
    ...
    print("=" * 72)
    print(
        f"  Server: {SERVER_HOST}:{SERVER_PORT}  |  Update: {UPDATE_INTERVAL*1000:.0f}ms ({1/UPDATE_INTERVAL:.0f}Hz)"
    )
    print()
