...
Serial Studio's USB driver talks to USB devices directly through **libusb**, bypassing the operating system's class drivers. It is the right driver when your device exposes vendor-specific bulk, control, or isochronous endpoints rather than presenting itself as a serial port (CDC), an HID device, or a mass-storage volume.

If your device shows up as a virtual COM port, use the [UART driver](Drivers-UART.md). If it's a gamepad, joystick, or HID firmware, use the [HID driver](Drivers-HID.md). This page is for everything else: logic analyzers, oscilloscopes, custom data acquisition boards, vendor-specific scientific instruments, and any device whose datasheet says "uses bulk endpoints."
...
Almost every consumer USB device implements one of the standard classes. The OS bundles a generic driver for each class, so plugging in any USB-CDC microcontroller gives you a COM port without installing anything.

**Vendor-specific** USB devices skip the standard classes. They expose endpoints with custom semantics, and the OS doesn't know what to do with them out of the box. That's where libusb comes in: a userspace library that lets an application open the device and talk to its endpoints directly.
...
- **Control transfers (Endpoint 0)** are used for configuration: reading device descriptors, selecting interfaces, sending vendor commands. Always bidirectional via the same endpoint pair.
...
- **Interrupt transfers** carry small amounts of data with **guaranteed maximum latency** (the polling interval set in the endpoint descriptor). Used for HID devices and other small periodic data. Despite the name, there are no actual hardware interrupts; the host polls.
- **Isochronous transfers** carry time-sensitive streaming data (audio, video) with **guaranteed bandwidth** but no retransmission. If a packet is corrupted, it's gone. Used for USB audio interfaces, webcams.

For Serial Studio's USB driver, the relevant ones are **bulk** (the default, for streaming captured data) and **isochronous** (for high-rate continuous streams where dropped frames are acceptable).
...
The host queries the device for its descriptor tree on enumeration, decides what to do with it, and binds drivers accordingly. For a vendor-specific bulk device, the descriptor tree usually has one configuration, one interface, and a couple of endpoints (one IN for reading, one OUT for writing).

### VID and PID

Every USB device has a **Vendor ID (VID)** and **Product ID (PID)**, both 16-bit. VIDs are assigned by the USB-IF; PIDs are picked by the vendor. Together they uniquely identify a device model. Serial Studio lists USB devices as `VID:PID, Product Name`.

## How Serial Studio uses it

The USB driver wraps libusb's asynchronous API. Setup is:

1. Pick a device by VID:PID.
2. Pick a **transfer mode**:
   - **Bulk Stream** (default): synchronous bulk IN/OUT. Works for most devices.
   - **Advanced Control**: bulk transfers plus vendor-specific control transfers. A confirmation dialog appears before enabling, because vendor-specific control writes can do anything the device firmware allows, including bricking it.
   - **Isochronous**: asynchronous isochronous transfers for fixed-rate streaming. You also configure the ISO packet size.
3. Pick the **IN endpoint** (and OUT endpoint, if writing).
4. Connect.
...
- **Permission denied (Linux), even with udev.** The udev rule didn't reload, or the device wasn't unplugged-and-replugged after the rule was added. Trigger a re-enumeration: `sudo udevadm trigger`.

## References

- [USB in a NutShell — Beyond Logic](https://www.beyondlogic.org/usbnutshell/usb-in-a-nutshell.shtml) — comprehensive introduction to USB.
